...
OV5640 的增益上限不是通用 camera enum 的 0 到 6。Espressif OV5640 驱动会把该值直接写入 0x3A18/0x3A19 并按 10-bit 原始值回读，因此看板使用十进制原始值 32、64、124、248、512、1023，对应约 2.0x、4.0x、7.75x、15.5x、32.0x、63.9x。旧的 0 到 6 会被拒绝，避免设置后回读成 248 之类的值导致控件回弹。
